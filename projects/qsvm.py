...
# ZZ & ZZ.T
SIZE = 6
fidelity_c = ZZFeatureMap(SIZE, reps=1, parameter_prefix='x')
ZZ_T = ZZFeatureMap(SIZE, reps=1, parameter_prefix='y').inverse()

fidelity_c.append(ZZ_T.decompose(),range(SIZE))
fidelity_c.measure_all()
fidelity_c = transpile(fidelity_c,optimization_level=3)
...
for i in range(num_jobs):
    ...
    log("Running Job: "+str(i)+"/"+str(num_jobs))
